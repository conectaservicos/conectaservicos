...
-- Criação automática do perfil após o cadastro no Supabase Auth.
create or replace function public.handle_new_user()
returns trigger language plpgsql security definer set search_path=public as $$
begin
  insert into public.profiles(id,full_name,email,phone,city,role,status)
  values(
    new.id,
    ...
    case when new.raw_user_meta_data->>'role'='professional' then 'professional' else 'client' end,
    case when new.raw_user_meta_data->>'role'='professional' then 'pending' else 'active' end
